...
# TODO: Implement Scale Factor Functionality
class Posit(object):
    def __init__(self, number = 0, nbits = None, es = None, rs = None, sc = None) -> None:
        if (nbits == None or es == None):
            raise Exception("Nbits and es are required")
        
        elif(rs == None and (sc == None or sc == 0)):
            self.nbits = nbits
            self.es = es
            self.rs = nbits - 1
            self.sc = 0
    ...
    def __ge__(self, other):
        if type(other) != Posit:
            other = Posit(other, nbits = self.nbits, es = self.es, rs = self.rs, sc = self.sc)
    ...
    def construct_posit(self, sign, scale, fraction):
        if fraction == 0:
            return Posit(nbits = self.nbits, es = self.es, rs = self.rs, sc=self.sc)
        ...
        if(self.rs < (self.nbits - 1)):
            ...
            if(exponent > (2**self.es - 1)):
                ...
                p = Posit(nbits = self.nbits, es = self.es, rs = self.rs, sc = self.sc)
...
def posify(x):
    return (Posit(x, 4, 1, 2))

vf = np.vectorize(posify)
if __name__ == "__main__":

    p = Posit(0,8,2)
    p.set_bit_pattern("00000010")
    print(p)
    p.set_float(float(p))
    print(p)
    p = Posit(0, 8,4,2)
    p.set_bit_pattern("00100000")
    print(p)

    x = (1*torch.rand(1, 128 , 128) + 10).cuda()
    x1 = deepcopy(x)
    x = (vf(x.cpu()))
    x = np.asarray(x, dtype=np.float32)
    x = torch.from_numpy(x).cuda()
    
    print("Input Number=",x1)
    print("Posit Number = ",x)

    print(F.mse_loss(x, x1))