...
def _quantize_elemwise_core(A, bits, exp_bits, max_norm, round='nearest',
    ...
    # Check if rounding mode is satisfied and only then use custom cuda
    custom_cuda = custom_cuda and round in RoundingMode.string_enums()

    # Uses provided cuda extensions
    if custom_cuda:
        A = A.contiguous()
...
if __name__ == "__main__":
    ...
    x = torch.randn(1, 2)
    max_norm = _get_max_norm(5, 5)
    print(x)
    print(_quantize_elemwise(x, 'fp4', ))