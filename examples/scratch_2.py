...
import numpy as np
import sys
sys.path.append("./number_system/mx")
...
def _shared_exponents_out(A, shared_exp_in, method="max", axes=None, ebits=0):
    """
    Get shared exponents for the passed matrix A.
    Args:
      A      {PyTorch tensor} -- Input tensor
    ...
    # Log2 computation to find the exponents
    shared_exp = torch.log2(
        shared_exp + FP32_MIN_NORMAL * (shared_exp == 0).type(shared_exp.dtype)  # Use a tiny positive to avoid log2(0)
    )

    # Now we truncate to integer and only take the top 4 bits of the exponent
    shared_exp = torch.floor(shared_exp)
    top4_bits = (shared_exp / 16).to(torch.int32) * 16  # This floors to the nearest multiple of 16, essentially masking out the lower 4 bits

    # Combine with the bottom 4 bits from shared_exp_in
    combined_exponent = top4_bits + (shared_exp_in % 16)  # Assumes shared_exp_in holds the valid lower 4 bits

    # Restrict to [-emax, emax] range
    if ebits > 0:
        emax = 2**(ebits-1) - 1
        combined_exponent.clamp_(-emax, emax)  # Use in-place clamp to reduce memory usage

    return combined_exponent
...
def _quantize_mx_outlier_v1(
    ...
    # print("SCALE EMAX IN", shared_exp_in)
    ...
    #print("SHARED EXP OUT", shared_exp_out)
...
if __name__ == "__main__":
    ...
    # Create a 2D tensor of size 5x5 filled with normal distributed values (mean=0, std=1)
    x = torch.randn(1, 5)

    # Introduce outliers by setting specific elements to extreme values
    outliers_indices = [(0, 3)]  # Indices where outliers will be placed
    for idx in outliers_indices:
        x[idx] = x[idx] * 1250  # Multiply by 50 to create outliers 

    '''
    def _quantize_mx_outlier(
    A,
    inlier_scale_bits,
    outlier_scale_bits,
    inlier_elem_format,    # can be None for no quantization
    ...
    print("With Outlier Handling = \t", _quantize_mx_outlier_v1(x, 4,4, 'fp4', 'int4', std_dev = 1, block_size=2, axes=[1], round="nearest"))
    print("Without Outlier Handling = \t", _quantize_mx(x, 4, 'fp4', block_size=2, axes=[1], round="nearest"))